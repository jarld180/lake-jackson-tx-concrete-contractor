...
const Navbar: React.FC = () => {
  ...
  const navLinks = [
    ...
    },
    {
      name: 'Locations',
      dropdown: [
        { name: 'Lake Jackson', path: '/' },
        { name: 'Angleton', path: '/angleton' },
        { name: 'Clute', path: '/clute' },
        { name: 'Freeport', path: '/freeport' },
        { name: 'Richwood', path: '/richwood' },
        { name: 'Brazoria', path: '/brazoria' }
      ]
    }
  ];
        ...
        <div className="p-8 h-full flex flex-col text-black">
          <div className="flex justify-between items-start mb-12 gap-4">
            <Link to={currentCity ? `/${currentCity}` : "/"} onClick={() => handleLinkClick(currentCity ? `/${currentCity}` : "/")} className="flex items-start space-x-2 min-w-0">
              ...
              <span className="text-xl font-bold uppercase tracking-tighter text-black truncate">Lake Jackson <span className="text-black">Concrete</span></span>
            </Link>
          ...
          <div className="mt-auto pt-8 border-t border-gray-100">
            ...
            <p className="text-center mt-4 text-black font-bold text-sm">(346) 676-8476</p>
          </div>
        </div>
      </div>
    </>
